import nimcrypto
import std/[strformat, strutils]

type 
  ## Context for AES256-GCM
  AESContext* = object 
    ectx, dctx: GCM[aes256]
    key: array[aes256.sizeKey, byte]
    iv: array[aes256.sizeBlock, byte]
    etag, dtag: array[aes256.sizeBlock, byte]

converter toSeqByte(s: string): seq[byte] = cast[seq[byte]](s)

proc initializeAes(ctx: var AESContext) = 
  discard randomBytes(addr ctx.iv[0], 16)
  discard randomBytes(addr ctx.key[0], 32)

proc format_nim_seq[I](buf: array[I, byte]) =
    var col = 0
    stdout.write &"var buf: array[{buf.len}, byte] = [ \n\tbyte  "
    for i in 0 ..< buf.len:
        if col > 11:
            col = 0
            stdout.write &"0x{buf[i].toHex},\n\t"
        elif i != buf.len-1:
            stdout.write &"0x{buf[i].toHex}, "
            col.inc
        else:
            stdout.write &"0x{buf[i].toHex}\n]\n"

proc display_ctx(ctx: AESContext) =
    stdout.write &"var iv  = [byte "
    for i in ctx.iv:
        stdout.write &"{i}, "
    echo "\b\b]"

    stdout.write &"var key  = [byte "
    for i in ctx.key:
        stdout.write &"{i}, "
    echo "\b\b]"

proc main() =
  # msfvenom -p windows/x64/exec CMD=calc.exe -f nim -e generic/none
  # modified byte to uint32, padded to mod 8
  var buf: array[280, byte] = [
    byte ]
  
  var 
    ctx: AESContext
    aadText: seq[byte] = "uscguscguscg"
    encText: array[buf.len, byte]
    
  initializeAes(ctx)
  ctx.ectx.init(ctx.key, ctx.iv, aadText)
  # encryption
  ctx.ectx.encrypt(buf, encText)
  ctx.ectx.clear()


  format_nim_seq(encText)
  display_ctx(ctx)

when isMainModule:
  main()